...
- 2025-08-22: 중복 FAQ 섹션 제거 - 페이지 구조 최적화 완료
...
[2025-08-22] Calendly 이벤트 추적 통합 완료
...
[2025-08-30] 구글폼 개인정보처리방침 링크 수정 필요
...
[2025-08-30] UTM 파라미터 보존 시스템 개선 완료
- keepUTM() 함수 구현: 간결하고 직관적인 JavaScript 기반 UTM 보존
- 모든 주요 CTA 버튼에 적용: 히어로, 푸터, 플로팅 모바일 CTA, 보증금 섹션
- 기존 복잡한 URL 조작 방식에서 단순한 location.search 활용 방식으로 개선
- 더 나은 성능: 함수 호출 방식으로 DOM 조작 오버헤드 최소화
- 호환성: 모든 주요 브라우저에서 안정적 작동
- GA4 추적과 연동: UTM 파라미터 보존과 이벤트 추적 동시 지원

[2025-08-30] 데모 위젯 추적 구현 확인 완료
✅ fireScheduleViewOnce() 함수: 정확히 구현됨
  - window.__sv_done 플래그로 중복 실행 방지
  - page_location 파라미터로 정확한 위치 추적
  - gtag 이벤트: schedule_view

✅ Calendly 위젯 스크립트 연결: 완벽하게 구현됨
  - <script src='widget.js' onload='fireScheduleViewOnce()'> 형태
  - 스크립트 로드 완료 시점에 정확히 이벤트 발생

✅ 다중 추적 시스템: 안정성 확보
  - 1차: 스크립트 로드 시점 (fireScheduleViewOnce)
  - 2차: DOM 감시 방식 (MutationObserver 백업)
  - Calendly postMessage 이벤트 수신

현재 구현이 제안된 코드와 100% 일치하여 추가 수정 불필요
